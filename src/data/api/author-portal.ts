...
export function useAuthorApps(scope: "workspace" | "registry", authorId: string) { return useQuery({ queryKey: ["author-apps", authorId, scope], queryFn: () => authFetch<{ items: AuthorApp[] }>(scope === "workspace" ? `/author-portal/authors/${encodeURIComponent(authorId)}/apps` : "/author-portal/admin/apps"), enabled: scope === "registry" || Boolean(authorId) }); }
