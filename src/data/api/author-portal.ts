...
export function useGitConnections(authorId: string) { return useQuery({ queryKey: ["author-git", authorId], queryFn: () => authFetch<{ items: GitConnection[] }>(`/author-portal/authors/${encodeURIComponent(authorId)}/git-connections`), enabled: Boolean(authorId) }); }
export function useAuthorApps(scope: "workspace" | "runtime", authorId: string) { return useQuery({ queryKey: ["author-apps", authorId, scope], queryFn: () => authFetch<{ items: AuthorApp[] }>(scope === "workspace" ? `/author-portal/authors/${encodeURIComponent(authorId)}/apps` : "/author-portal/admin/runtime/apps"), enabled: scope === "runtime" || Boolean(authorId) }); }
export function useCatalogSubmissions(scope: "workspace" | "registry", authorId: string) { return useQuery({ queryKey: ["author-catalog", authorId, scope], queryFn: () => authFetch<{ items: CatalogSubmission[]; operator: boolean }>(scope === "workspace" ? `/author-portal/authors/${encodeURIComponent(authorId)}/catalog-submissions` : "/author-portal/admin/catalog-submissions"), enabled: scope === "registry" || Boolean(authorId) }); }
export function useAuthorActivity(authorId: string) { return useQuery({ queryKey: ["author-activity", authorId], queryFn: () => authFetch<{ items: ActivityEvent[] }>(`/author-portal/authors/${encodeURIComponent(authorId)}/activity`), enabled: Boolean(authorId) }); }
export function useGitRepositories(authorId: string, connectionId: string) { return useQuery({ queryKey: ["author-git-repos", authorId, connectionId], queryFn: () => authFetch<{ items: GitRepository[] }>(`/author-portal/authors/${encodeURIComponent(authorId)}/git-connections/${encodeURIComponent(connectionId)}/repositories`), enabled: Boolean(authorId && connectionId), retry: false }); }
export function useAuthorMembers(authorId: string) { return useQuery({ queryKey: ["author-members", authorId], queryFn: () => authFetch<{ items: AuthorMember[] }>(`/author-portal/authors/${encodeURIComponent(authorId)}/members`), enabled: Boolean(authorId), retry: false }); }
...
export const authorPortalRequests = {
  ...
  saveMember: (payload: { authorId: string; user_id: string; role: string }) => authFetch<AuthorMember>(`/author-portal/authors/${encodeURIComponent(payload.authorId)}/members`, { method: "PUT", body: JSON.stringify({ user_id: payload.user_id, role: payload.role }) }),
  connectGitHub: (payload: { author_id: string; token: string }) => authFetch<GitConnection>(`/author-portal/authors/${encodeURIComponent(payload.author_id)}/git-connections/github`, { method: "POST", body: JSON.stringify({ token: payload.token }) }),
  disconnectGit: (payload: { authorId: string; connectionId: string }) => authFetch<void>(`/author-portal/authors/${encodeURIComponent(payload.authorId)}/git-connections/${encodeURIComponent(payload.connectionId)}`, { method: "DELETE" }),
  inspectRepository: (payload: { author_id: string; connection_id: string; repository: string; branch: string; manifest_path: string }) => authFetch<{ manifest: Record<string, unknown> | null; errors: string[]; status: string }>(`/author-portal/authors/${encodeURIComponent(payload.author_id)}/repositories/inspect`, { method: "POST", body: JSON.stringify(payload) }),
  createApp: (payload: { author_id: string; connection_id: string; repository: string; branch: string; manifest_path: string }) => authFetch<AuthorApp>(`/author-portal/authors/${encodeURIComponent(payload.author_id)}/apps/from-git`, { method: "POST", body: JSON.stringify(payload) }),
  appAction: (payload: { authorId?: string; appId: string; action: string; notes?: string }) => authFetch<AuthorApp>(payload.authorId ? `/author-portal/authors/${encodeURIComponent(payload.authorId)}/apps/${encodeURIComponent(payload.appId)}/action` : `/author-portal/admin/runtime/apps/${encodeURIComponent(payload.appId)}/action`, { method: "POST", body: JSON.stringify({ action: payload.action, notes: payload.notes }) }),
  submitCatalog: (payload: { authorId: string; appId: string }) => authFetch<CatalogSubmission>(`/author-portal/authors/${encodeURIComponent(payload.authorId)}/apps/${encodeURIComponent(payload.appId)}/catalog-submission`, { method: "POST" }),
