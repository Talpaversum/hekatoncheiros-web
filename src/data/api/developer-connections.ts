...
export type DeveloperConnection = { connection_id: string; tenant_id: string; provider: "github"|"gitlab"|"git"|"local_workspace"|"private_feed"; auth_method: string; owner_label: string; status: "pending"|"verified"|"error"|"revoked"; scopes: string[]; metadata: Record<string,unknown>; has_credential: boolean; last_used_at: string|null; last_verified_at: string|null; created_at: string; updated_at: string };
...
export const developerConnectionRequests={
  create:(input:{provider:DeveloperConnection["provider"];auth_method:string;owner_label:string;scopes:string[];credential?:string;metadata:Record<string,unknown>})=>authFetch<DeveloperConnection>("/developer-connections",{method:"POST",body:JSON.stringify(input)}),
