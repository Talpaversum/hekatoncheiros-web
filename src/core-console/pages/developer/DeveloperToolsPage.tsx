...
function ProjectSummary({ project }: { project?: DeveloperProject }) {
  ...
  return <div className="space-y-4"><Card><SectionHeader title={project.display_name} description={t("developerProject.projectEntityDescription")} /><dl className="grid gap-3 text-sm md:grid-cols-3"><Item label={t("developerProject.sourceRevision")} value={project.source_revision ?? "-"} /><Item label={t("developerProject.validatedRevision")} value={project.validated_revision ?? "-"} /><Item label={t("developerProject.deployedRevision")} value={project.deployed_revision ?? "-"} /><Item label={t("developerProject.deploymentStatus")} value={project.deployment_status} /><Item label={t("developerProject.runtime")} value={project.runtime_status} /><Item label={t("developerProject.installedApp")} value={project.installed_app_id ?? "-"} /></dl></Card>{diff.data&&<Card><SectionHeader title={t("developerProject.changePreview")} description={t("developerProject.changePreviewDescription")} /><div className="grid gap-3 sm:grid-cols-2"><Result tone={diff.data.manifest.changed?"danger":"success"}>{t(diff.data.manifest.changed?"developerProject.manifestChanged":"developerProject.manifestUnchanged")}</Result><Result tone={diff.data.runtime.changed?"danger":"success"}>{t(diff.data.runtime.changed?"developerProject.runtimeChanged":"developerProject.runtimeUnchanged")}</Result></div><pre className="mt-3 max-h-80 overflow-auto bg-hc-bg p-3 text-xs">{JSON.stringify(diff.data,null,2)}</pre></Card>}</div>;
}
...
function AddProjectWorkflow() {
  ...
  return <div className="space-y-5">
    ...
    {wizardStep === 3 && <Step number={3} title={t("developerProject.wizard.discovery")} state="active"><Field label={t("developerProject.manifestPath")}><Input value={manifestPath} onChange={(event)=>setManifestPath(event.target.value)} /></Field><p className="text-sm text-hc-muted">{t("developerProject.discoveryHint")}</p></Step>}
